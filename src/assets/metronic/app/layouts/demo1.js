class KTLayout {
  ...
  static _handleSidebar() {
    const sidebarToggle = window.KTToggle?.getInstance(this.sidebarToggleEl)
    ...
    sidebarToggle?.on('toggle', () => {
      ...
      window.KTDom?.transitionEnd(this.sidebarEl, () => {
  ...
  static _handleSidebarMenu() {
    ...
    if (!menuActiveItemEl || !window.KTDom?.isVisibleInParent(menuActiveItemEl, scrollableEl)) {
      return
    }

    scrollableEl.scroll({
      top: window.KTDom?.getRelativeTopPosition(menuActiveItemEl, scrollableEl) - 100,
      behavior: 'instant',
    })
  }
